...
SRes LZMAEncodeFile(char* input, char* output)
{
    ...
    RINOK(InFile_Open(&inStream.file, input) != 0);
    RINOK(OutFile_Open(&outStream.file, output) != 0);
...
SRes LZMADecodeFile(char* input, char* output)
{
    CFileSeqInStream inStream;
    CFileOutStream outStream;
    int res;
